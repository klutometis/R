  ...
  (receive (named-args unnamed-args)
    ...
    (let ((named-args (map (match-lambda ((name . arg)
      ...
      ((foreign-lambda*
        ...
        ;; "  puts(C_c_string(C_i_caar(named)));"
        "  printf(\"typeof: %d\\n\", TYPEOF((SEXP) C_c_pointer_or_null(C_i_cdar(named)))); "
        ;; "  SET_TAG(arg, install(C_c_string(C_i_caar(named))));"
        ;; "  SET_TAG(arg, install(\"envir\"));"
        ;; "  R_tryEval(Rf_lang2(install(\"debug\"), arg), R_GlobalEnv, &error); "
        "  SETCAR(ei, arg);"
        "  named = C_i_cdr(named);"
        "  ei = CDR(ei);"
        ;; "  SETCAR(expression, Rf_findFun(install(\"typeof\"), R_GlobalEnv)); "
        ;; "  SETCAR(expression, Rf_findFun(install(\"debug\"), R_GlobalEnv)); "
        "}"
        ;; "puts(\"BAGGIES\"); "
        ;; "SETCAR(expression, Rf_findFun(install(\"str\"), R_GlobalEnv)); "
        ;; "R_tryEval(Rf_lang2(install(\"str\"), expression), R_GlobalEnv, &error); "
        ;; "R_tryEval(Rf_lang2(install(\"debug\"), expression), R_GlobalEnv, &error); "
