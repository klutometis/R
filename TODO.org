...
* TODO What does =NULL= correspond to?
...
# <<less-in-c>>
  ...
  #+BEGIN_SRC scheme :tangle R-less-c.scm
    ...
    (define-foreign-variable R-null int "NILSXP")
    (define-foreign-variable R-boolean int "LGLSXP")
    (define-foreign-variable R-integer int "INTSXP")
    (define-foreign-variable R-real int "REALSXP")
    (define-foreign-variable R-real int "CPLXSXP")
    (define-foreign-variable R-string int "STRSXP")
    (define-foreign-variable R-list int "VECSXP")
    ...
    (define R-null? (cut R-type? <> R-null))
    (define R-boolean? (cut R-type? <> R-boolean))
    (define R-integer? (cut R-type? <> R-integer))
    (define R-real? (cut R-type? <> R-real))
    (define R-complex? (cut R-type? <> R-complex))
    (define R-string? (cut R-type? <> R-string))
    (define R-list? (cut R-type? <> R-list))
    ...
    (define (R-boolean-ref vector i)
      ((foreign-lambda*
        ...
        ((SEXP vector)
         (int i))
        "C_return(LOGICAL((SEXP) vector)[i] ? C_SCHEME_TRUE : C_SCHEME_FALSE);")
       vector
       i))
    ...
    (define (R-car list)
      ((foreign-lambda* SEXP
                        ((SEXP list))
                        "C_return(CAR((SEXP) list));")
       list))
    (define (R-cdr list)
      ((foreign-lambda* SEXP
                        ((SEXP list))
                        "C_return(CDR((SEXP) list));")
       list))
    ...
    (define (R->scheme value)
      ...
      (let ((length (R-length value)))
        ...
        (match value
          ...
          ((? R-integer?)
           (if (= length 1)
               (R-integer-ref value 0)
               (do ((vector (make-vector length))
                    (i 0 (+ i 1)))
                   ((= i length) vector)
                 (vector-set! vector i (R-integer-ref value i)))))
          ((? R-list?)
           (do ((vector (make-vector length))
                (i 0 (+ i 1)))
               ((= i length) vector)
             (vector-set! vector i (R->scheme (R-vector-ref value i)))))
          ((? R-string?)
           (if (= length 1)
               (R-string-ref value 0)
               (do ((vector (make-vector length))
                    (i 0 (+ i 1)))
                   ((= i length) vector)
                 (vector-set! vector i (R-string-ref value i))))))))
