...
# <<less-in-c>>
  ...
  #+BEGIN_SRC scheme :tangle R-less-c.scm
    ...
    static void gc_mutation (C_word *slot, C_word val) {
      puts ("marro!");
           fflush (stdout);
    }
    
    static void gc_end (int mode, long ms) {
     puts ("mein-gott-full-of-stars");
          fflush (stdout);
    }
    <#
    
    (define interesting-objects (make-hash-table))
    ...
    (foreign-code
     ...
     Rf_initEmbeddedR(4, (char*[]) {"R-less-c",
                                    ...
                                    "--vanilla",
                                    "--args"});
     C_gc_mutation_hook = gc_mutation;
     C_post_gc_hook = gc_end;
    END
    )
    ...
    (define-foreign-type SEXP
      ...
      (lambda (sexp)
        ((foreign-lambda*
         ...
         sexp)
        (hash-table-set! interesting-objects (object->pointer sexp) sexp)
        (debug 'pre-gc (hash-table->alist interesting-objects))
        (gc #t)
        (debug 'post-gc (hash-table->alist interesting-objects))
        (set-finalizer!
